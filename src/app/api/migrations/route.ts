...
const MIGRATIONS = [
  ...
  },
  {
    ...
    sql: `
      ...
      CREATE POLICY "Users can view own IPs" ON user_ip_tracking FOR SELECT USING (auth.uid() = user_id);
      
      DROP POLICY IF EXISTS "Service role full access" ON user_ip_tracking;
      CREATE POLICY "Service role full access" ON user_ip_tracking FOR ALL USING (auth.role() = 'service_role');
